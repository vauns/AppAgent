...
def parse_explore_rsp(rsp):
    try:
        ...
        action = re.findall(r"Next Action: (.*?)$", rsp, re.MULTILINE)[0]
        operation = re.findall(r"Grounded Operation:(.*?)$", rsp, re.MULTILINE)[0]

        print_with_color("Plan:", "yellow")
        print_with_color(plan, "magenta")
        print_with_color("Action:", "yellow")
        print_with_color(action, "magenta")
        print_with_color("Operation:", "yellow")
        print_with_color(operation, "magenta")
        if "FINISH" in action:
            return ["FINISH"]

        action_name = re.search(r"type (.*)", operation)
        if action_name is None:
            action_name = re.findall(r"(.*) \[\[", operation)[0].lower()
        else:
            action_name = "text"

        if "-> click at the box" in action_name:
            normalized = re.findall(r"-> CLICK at the box \[\[(.*)]]", operation)[0]
            x_min, y_min, x_max, y_max = map(int, normalized.split(','))
            x_center = (x_min + x_max) / 2
            y_center = (y_min + y_max) / 2
            normalized = ",".join(map(str, [x_center, y_center]))
            return ["tap", normalized]
        elif action_name == "tap":
            normalized = re.findall(r"tap \[\[(.*)]]", operation)[0]
            return [action_name, normalized]
        elif action_name == "text":
            display_name = re.findall(r"'(.*?)'", action)[0]
            input_str = re.findall(r"type (.*)", operation)[0]
            return [action_name, input_str, display_name]
        ...
        #     return [act_name, area, swipe_dir, dist, last_act]
            ...
            print_with_color(f"ERROR: Undefined act {action_name}!", "red")
            return ["ERROR"]
